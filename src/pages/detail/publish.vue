    ...
    <my-dialog :is-show="isShowPayDialog" @on-close = 'hidePayDialog'>
      <table class="buy-dialog-table">
        ...
        <tr>
          <td> {{buyNum}}</td>
          <td> {{buyType.label}} </td>
          <td><span v-for="item in versions" :key="item.id">{{item.label}}, </span></td>
          <td> 一年 </td>
          <td> {{ price }} </td>
        </tr>
      </table>
...
<script>
import VSelection from '../../components/base/selelction'
...
export default {
  ...
  methods: {
    ...
    getPrice () {
      ...
      // )
      // 本地计算版
      this.calculatePrice(reqParams)
    },
    calculatePrice (params) {
      let totalPrice, num, type, version
      num = params.buyNumber
      if (params.buyType === 0) {
        type = 10
      } else if (params.buyType === 1) {
        type = 20
      } else if (params.buyType === 2) {
        type = 30
      } else if (params.buyType === 3) {
        type = 40
      } else {
        type = 50
      }
      ...
      version = 0
      if (params.version.indexOf('0') > -1) {
        version += 1
      }
      if (params.version.indexOf('1') > -1) {
        version += 2
      }
      if (params.version.indexOf('2') > -1) {
        version += 3
      }
      if (params.version.indexOf('3') > -1) {
        version += 4
      }
      totalPrice = num * type * version
